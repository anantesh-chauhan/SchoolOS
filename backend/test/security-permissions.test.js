...
test('Curriculum Manager receives academic permissions but not credential administration', () => {
  ...
  assert.equal(hasPermission('CURRICULUM_MANAGER', PERMISSIONS.CALENDAR_MANAGE), true);
  assert.equal(hasPermission('CURRICULUM_MANAGER', PERMISSIONS.USERS_PASSWORD_RESET), false);
});

test('school administrators receive explicit school-scoped control', () => {
...
test('platform owners are restricted to platform operations outside audited support mode', () => {
  assert.equal(hasPermission('PLATFORM_OWNER', PERMISSIONS.PLATFORM_SETTINGS_MANAGE), true);
  assert.equal(hasPermission('PLATFORM_OWNER', PERMISSIONS.CURRICULUM_MANAGE), false);
  assert.equal(hasPermission('PLATFORM_OWNER', PERMISSIONS.FEES_REPORT), false);
});
