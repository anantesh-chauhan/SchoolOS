...
const row = (attendanceDate, status, attendanceUnits) => ({ attendanceDate: date(attendanceDate), status, attendanceUnits });

test('attendance percentage uses attendance units and eligible working days', () => assert.equal(percentage(17.5, 20), 87.5));
test('percentage is not calculated when there are no eligible working days', () => assert.equal(percentage(0, 0), null));
test('half-day calculation is centralized and configurable', () => { assert.equal(statusWeight('HALF_DAY'), .5); assert.equal(statusWeight('HALF_DAY', { halfDayWeight: .25 }), .25); });
test('weekly offs and holidays are excluded from working days', () => { const days=buildWorkingDays({start:date('2026-07-01'),end:date('2026-07-08'),weeklyOffDays:[0],calendarDays:[{calendarDate:date('2026-07-03'),dayType:'HOLIDAY',isFullDay:true}]});assert.equal(days.filter(day=>day.isWorkingDay).length,5);assert.equal(days.find(day=>day.date==='2026-07-03').isWorkingDay,false); });
test('special working day overrides a weekly off', () => { const days=buildWorkingDays({start:date('2026-07-05'),end:date('2026-07-06'),weeklyOffDays:[0],overrides:[{calendarDate:date('2026-07-05'),isWorkingDay:true,eligibleDayWeight:.5}]});assert.deepEqual(days[0],{date:'2026-07-05',isWorkingDay:true,weight:.5,dayType:'WORKING_DAY',title:null}); });
test('new admission excludes days before effective joining date', () => { const days=buildWorkingDays({start:date('2026-07-01'),end:date('2026-07-06'),weeklyOffDays:[]});const result=summarizeAttendance({rows:[row('2026-07-03','PRESENT',1),row('2026-07-04','PRESENT',1),row('2026-07-05','ABSENT',0)],workingDays:days,enrollment:{effectiveFrom:date('2026-07-03')}});assert.equal(result.eligibleWorkingDays,3);assert.equal(result.percentage,66.7); });
...
test('submitted attendance cannot return to draft', () => { assert.equal(canTransitionAttendance('DRAFT','LOCKED'),true); assert.equal(canTransitionAttendance('LOCKED','CORRECTED'),true); assert.equal(canTransitionAttendance('CORRECTED','CORRECTED'),true); assert.equal(canTransitionAttendance('LOCKED','DRAFT'),false); assert.throws(() => assertAttendanceTransition('LOCKED','DRAFT'),/cannot transition/); });
test('tenant isolation rejects cross-school access', () => assert.throws(() => assertSameSchool({role:'ADMIN',schoolId:'school-a'},'school-b'),/another school's data/));
