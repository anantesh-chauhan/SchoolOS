...
export const getStudentFees = async (user, requestedStudentId, academicSession) => {
  ...
  const assignments = await prisma.feeAssignment.findMany({
    where: { schoolId, academicSession: session, active: true, OR: [
      { studentId }, { targetType: 'SCHOOL' }, { targetType: 'CLASS', targetValue: student.className },
      { targetType: 'SECTION', targetValue: { in: [student.section || '', `${student.className}:${student.section || ''}`] } },
    ] },
  ...
  return json({ student, account, totals, assignedStructure: assignments[0]?.feeStructure || null, assignment: assignments[0] || null, onlinePayment: { enabled: false, label: 'Coming Soon' } });
};

export const getFeeHierarchy = async (user, academicSession) => {
  const schoolId = tenant(user);
  const [classes, students] = await Promise.all([
    prisma.class.findMany({ where: { schoolId, deletedAt: null }, include: { sections: { where: { deletedAt: null }, orderBy: { sectionOrder: 'asc' } } }, orderBy: { classOrder: 'asc' } }),
    prisma.student.findMany({ where: { schoolId, isActive: true, ...(academicSession ? { session: academicSession } : {}) }, include: { feeAccounts: { where: academicSession ? { academicSession } : undefined, include: { charges: true }, take: 1 } }, orderBy: [{ className: 'asc' }, { section: 'asc' }, { rollNumber: 'asc' }, { studentFirstName: 'asc' }] }),
  ]);
  const summarize = (student) => {
    const account = student.feeAccounts[0];
    const totals = (account?.charges || []).reduce((sum, charge) => { const c = calculateCharge(charge); sum.expected += c.netMinor; sum.paid += BigInt(charge.paidMinor); sum.pending += c.payableMinor; if (charge.status === 'OVERDUE') sum.overdue += c.payableMinor; return sum; }, { expected: 0n, paid: 0n, pending: 0n, overdue: 0n });
    return { id: student.id, name: `${student.studentFirstName} ${student.studentLastName || ''}`.trim(), admissionNo: student.admissionNo, studentUserId: student.studentUserId, rollNumber: student.rollNumber, parentName: student.fatherName, parentMobile: student.parentMobile, className: student.className, section: student.section, session: student.session, feeStatus: totals.expected === 0n ? 'NOT_ASSIGNED' : totals.pending === 0n ? 'PAID' : totals.paid > 0n ? 'PARTIALLY_PAID' : totals.overdue > 0n ? 'OVERDUE' : 'PENDING', totals };
  };
  const addTotals = (rows) => rows.reduce((sum, row) => ({
  ...
  const hierarchy = classes.map((classRow) => {
    const sections = classRow.sections.map((section) => {
      const sectionStudents = students.filter((student) => student.className === classRow.className && student.section === section.sectionName).map(summarize);
      return { id: section.id, sectionName: section.sectionName, students: sectionStudents, totals: addTotals(sectionStudents) };
    });
    return { id: classRow.id, className: classRow.className, sections, totals: addTotals(sections) };
  });
  const unplaced = students.filter((student) => !classes.some((classRow) => classRow.className === student.className && classRow.sections.some((section) => section.sectionName === student.section))).map(summarize);
  return json({ academicSession, classes: hierarchy, unplaced, totals: { classes: hierarchy.length, sections: hierarchy.reduce((count, row) => count + row.sections.length, 0), students: students.length } });
};
