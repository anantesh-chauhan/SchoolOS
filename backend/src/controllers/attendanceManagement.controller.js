...
export const updateAttendanceSettings = async (req, res) => {
  try {
    ...
    const allowed = ['timezone','academicSessionStart','academicSessionEnd','weeklyOffDays','markingDeadline','attendanceOpenTime','classTeacherDeadline','adminAlertTime','finalSubmissionTime','autoLockTime','allowBackdatedAttendance','maximumBackdatedDays','prohibitFutureAttendance','allowDraft','defaultAttendanceStatus','requireAdminOverrideReason','notifyClassTeacherOnCorrection','correctionWindowHours','studentMinimumPercentage','employeeMinimumPercentage','halfDayWeight','lateWeight','approvedLeaveWeight','medicalLeaveWeight','consecutiveAbsenceAlertDays','parentAbsenceNotifications','studentNotifications','requiresFinalSubmission','automaticMonthEndLock','correctionsRequireAdminApproval','payrollIntegrationEnabled'];
    ...
    const timeFields = ['markingDeadline','attendanceOpenTime','classTeacherDeadline','adminAlertTime','finalSubmissionTime','autoLockTime'];
    if (timeFields.some((key) => data[key] !== undefined && !/^([01]\d|2[0-3]):[0-5]\d$/.test(String(data[key])))) return fail(res, 400, 'Attendance times must use 24-hour HH:mm format');
    if (data.maximumBackdatedDays !== undefined && (!Number.isInteger(Number(data.maximumBackdatedDays)) || Number(data.maximumBackdatedDays) < 0 || Number(data.maximumBackdatedDays) > 365)) return fail(res, 400, 'Maximum backdated days must be between 0 and 365');
...
export const saveStudentDailyRegister = async (req, res) => {
  try {
    const targetState = req.body.state === 'SUBMITTED' ? 'SUBMITTED' : 'DRAFT';
    const workflow = await saveStudentAttendanceSession({ user: req.user, sectionId: req.body.sectionId, attendanceDate: req.body.date, payload: req.body, targetState, request: req });
    return res.json({ success: true, message: workflow.message, data: workflow });
    /* istanbul ignore next -- retained temporarily as unreachable legacy reference during API compatibility period */
...
export const reviewCorrectionRequest=async(req,res)=>{
  try{
    ...
    if(decision==='APPROVED'&&request.personType==='STUDENT'){
      const attendance=await prisma.studentAttendance.findFirst({where:{id:request.studentAttendanceId,schoolId:req.user.schoolId}});if(!attendance)return fail(res,404,'Student attendance record not found');
      const register=await prisma.attendanceDailyRegister.findUnique({where:{schoolId_classId_sectionId_attendanceDate:{schoolId:req.user.schoolId,classId:attendance.classId,sectionId:attendance.sectionId,attendanceDate:attendance.attendanceDate}}});if(!register)return fail(res,409,'Submitted attendance session not found');
      await correctStudentAttendanceSession({user:req.user,attendanceSessionId:register.id,payload:{reasonCode:'APPROVED_CORRECTION_REQUEST',reasonNote:clean(req.body.reviewRemark)||request.reason,expectedRevision:register.currentRevisionNumber,changes:[{studentId:request.personId,newStatus:request.requestedStatus,newRemark:attendance.remarks}]},request:req});
    }
    await prisma.$transaction(async tx=>{if(decision==='APPROVED'&&request.personType==='EMPLOYEE')await tx.employeeAttendance.update({where:{id:request.employeeAttendanceId},data:{status:request.requestedStatus,attendanceUnits:statusWeight(request.requestedStatus,{},definitions)}});const saved=await tx.attendanceCorrectionRequest.update({where:{id:request.id},data:{status:decision,reviewedById:req.user.id,reviewedAt:new Date(),reviewRemark:clean(req.body.reviewRemark)||null}});await tx.attendanceAuditLog.create({data:auditData(req,`CORRECTION_${decision}`,'ATTENDANCE_CORRECTION',request.id,request,saved,req.body.reviewRemark)});});
